...
func main() {
	...
	time.Sleep(3200 * time.Millisecond) // set a delay/wait interval that allows previous functions be executed. Blocking element.
	fmt.Println("\nmain terminated")
}
...
func numbers() {
	...
	//
	rand.Seed(time.Now().UnixNano()) //it randomizes the number
